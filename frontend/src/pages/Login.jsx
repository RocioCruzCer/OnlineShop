...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      // Consumimos el endpoint GET /api/usuarios desde Railway
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true);
    try {
      // Consumimos el endpoint POST /api/usuarios/registro desde Railway
  ...
  return (
      <div className="login-container">
        <div className="login-card">
          ...
          {error && <div className="alert alert-error" style={{ marginBottom: '16px', color: '#d32f2f' }}>{error}</div>}
          ...
          <p style={{ color: 'var(--text-dim)', fontSize: '0.85rem', marginTop: '18px', textAlign: 'center' }}>
            {mode === 'login' ? (
                <>¿No tienes cuenta? <button className="link-btn" style={{ background: 'none', border: 'none', color: '#007bff', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => { resetForm(); setMode('register'); }}>Regístrate</button></>
            ) : (
                <>¿Ya tienes cuenta? <button className="link-btn" style={{ background: 'none', border: 'none', color: '#007bff', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => { resetForm(); setMode('login'); }}>Inicia sesión</button></>
            )}
          </p>
        </div>
      </div>
